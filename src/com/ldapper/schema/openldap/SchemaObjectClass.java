...
public class SchemaObjectClass {
	...
	public String toSchemaString() {
		...
		buffer.append("objectclass ( ").append(oid).append(' ').append(CR);
		...
		if (aux.size() > 0) {
			buffer.append(TAB).append("SUP").append(" ( ");
		...
		if (type != null) {
			buffer.append(TAB).append(type).append(' ').append(CR);
		}
